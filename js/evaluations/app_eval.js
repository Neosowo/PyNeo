...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateUserColorBasedOnDifficulty(); 
    renderEvaluations();
});
...
function updateUserColorBasedOnDifficulty(difficulty) {
    ...
    const introCompleted = modulesProgress[1] || modulesProgress['1']; 
    ...
    Object.values(evalProgress).forEach(p => {
        if (p.completed && priorities[p.difficulty] > highest) {
            ...
            if (p.difficulty === 'intermedio') userColor = '#eab308'; 
            if (p.difficulty === 'avanzado') userColor = '#ef4444'; 
            if (p.difficulty === 'experto') userColor = '#a855f7'; 
            if (p.difficulty === 'máster') userColor = '#06b6d4'; 
            if (p.difficulty === 'oculto') userColor = 'glitch-effect'; 
        }
    });
